...
export default function PricingPage() {
  ...
  async function saveQuotePackage(pkg: ProtectionPackage) {
    if (!vehicle) return;
    setSavingMsg("Saving…");
    const res = await fetch("/api/quotes", {
      ...
      body: JSON.stringify({
        ...
        packageSnapshot: pkg,
        retailTotal: pkg.retail_total,
        costTotal: pkg.cost_total,
        grossTotal: pkg.gross_total,
      }),
    });
    const data = await res.json();
    if (res.ok) {
      setSavingMsg(`Quote saved · ${data.quote.quote_number}`);
    } else {
  ...
  async function saveAlacarte() {
    if (!vehicle || totals.count === 0) return;
    setSavingMsg("Saving…");
    const items = Array.from(selected).map((k) => acIndex[k]);
    const res = await fetch("/api/quotes", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    const data = await res.json();
    if (res.ok) {
      setSavingMsg(`À la carte quote saved · ${data.quote.quote_number}`);
    } else {
      setSavingMsg(data.error || "Save failed.");
    }
    setTimeout(() => setSavingMsg(""), 4000);
        ...
        <section className="stage">
          ...
          {pricing?.packages.map((p) => {
            ...
            return (
              <div
                key={p.id}
                className={`package${expanded ? " selected" : ""}`}
